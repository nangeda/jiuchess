#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Basic Enhanced vs Expert 100局对战 - 详细输出版"""
...
def play_game_detailed(agent, expert, agent_is_white, game_num):
    ...
    print(f"\n{'═'*60}", flush=True)
    print(f"📍 第 {game_num} 局 | Basic增强({agent_color}) vs Expert({expert_color})", flush=True)
    ...
    while not state.is_over() and step < 800:
        ...
        if step % 50 == 0 and step > last_report:
            ...
            print(f"  Step {step:4d} [{phase}] | 白:{w_count:2d} 黑:{b_count:2d} | "
                  f"用时 Basic:{avg_agent:.2f}s Expert:{avg_expert:.2f}s", flush=True)
    ...
    print(f"{'─'*60}", flush=True)
    winner_name = "Basic增强" if result == 'agent' else ("Expert" if result == 'expert' else "平局")
    print(f"🏁 第 {game_num} 局结束 | 获胜: {winner_name}", flush=True)
    print(f"   最终子数: 白 {w_count} vs 黑 {b_count}", flush=True)
    print(f"   平均走子时间: Basic {avg_agent_total:.3f}s, Expert {avg_expert_total:.3f}s", flush=True)
...
def main():
    print("═" * 60, flush=True)
    print("🎯 Basic Enhanced vs Expert 对战", flush=True)
    print("═" * 60, flush=True)
    ...
    agent = BasicEnhancedAgent('exp/jcar_sft_2025_balanced/checkpoint_best.pt', device='cuda:0')
    print("✅ 加载完成\n", flush=True)
    ...
    for i in range(1, 101):
        ...
        print(f"\n  【当前战绩】Basic增强 {wins} 胜 | Expert {losses} 胜 | 平局 {draws} | 胜率 {win_rate:.1f}%\n", flush=True)
    ...
    print("═" * 60, flush=True)
    print(f"  Basic Enhanced: {wins} 胜 ({wins}%)", flush=True)
